...
class Day03
{
    static int Part1()
    {
        ...
        int[][] banks = bankStrs.Select(line => line.ToCharArray().Select(c => int.Parse(c.ToString())).ToArray()).ToArray();
        ...
        {
            int[] bank = banks[i];
            int[] firstCandidates = bank.Take(bank.Length - 1).ToArray();
            int first = firstCandidates.Max();
            int firstIndice = Array.IndexOf(bank, first);
            int[] secondCandidates = bank.Skip(firstIndice + 1).ToArray();
    ...
    static BigInteger Part2()
    {
        ...
        for (int i = 0; i < banks.Length; i++)
        {
            ...
            for (int n = 1; n <= 12; n++)
            {
                int[] candidates = bank.Take(bank.Length - (12 - n)).ToArray();
                int digit = candidates.Max();
                int digitIndice = Array.IndexOf(candidates, digit);
                bank = bank.Skip(digitIndice + 1).ToArray();
